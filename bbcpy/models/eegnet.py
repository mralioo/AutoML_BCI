...
from bbcpy.models.utils.functions import squeeze_final_output
import optuna
...
class EEGNetv4(nn.Sequential):
    ...
    def __init__(
        ...
        if isinstance(trial, optuna.Trial):
            self.F1 = trial.suggest_int("F1", low=6, high=20, step=2)
            self.D = trial.suggest_int("D", low=2, high=10, step=2)
            self.F2 = trial.suggest_int("F2", low=10, high=20, step=2)
            self.kernel_length = trial.suggest_int("kernel_length", low=20, high=80, step=10)
            self.drop_prob = trial.suggest_float("dropout_rate", 0, 1)
            self.pool_mode = trial.suggest_categorical("pool_mode", ["mean", "max"])
        else :
            self.F1 = F1
            self.D = D
            self.F2 = F2
