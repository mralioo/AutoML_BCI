...
from bbcpy.models.utils.modules import Expression, AvgPool2dWithConv, Ensure4d
from bbcpy.models.utils.functions import identity, transpose_time_to_spat, squeeze_final_output
...
class Deep4Net(nn.Sequential):
    ...
    def __init__(
        ...
        if isinstance(trial, optuna.Trial):
            self.n_filters_time = trial.suggest_int("n_filters_time", low=25, high=70, step=15)
            self.filter_time_length = trial.suggest_int("filter_time_length", low=10, high=30, step=10)
            self.n_filters_spat = trial.suggest_int("n_filters_spat", low=25, high=70, step=15)
            self.pool_time_length = trial.suggest_int("pool_time_length", low=1, high=5, step=2)
            self.pool_time_stride = trial.suggest_int("pool_time_stride", low=1, high=5, step=2)
            self.n_filters_2 = trial.suggest_int("n_filters_2", low=25, high=70, step=15)
