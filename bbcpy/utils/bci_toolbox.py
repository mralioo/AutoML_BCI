...
def crossvalidation(classifier_fcn, X, y,NumComp, folds=10, verbose=False, feature_extraction=None):
    '''
    Synopsis:
    ...
    for ff in range(folds):
        ...
        fv=feature_extraction(X,y,idxTr,NumComp)
...
def train_TDSEP(cnt,tau=[0, 1]):
    ...
    if tau[0]==0:        
        ...
        S_T1=(np.dot(cnt[:,:-tau[0]],cnt[:,tau[0]:].T)+np.dot(cnt[:,tau[0]:],cnt[:,:-tau[0]].T))/(cnt.shape[1]-1-tau[0])/2
    if tau[1]==0:        
        S_T2=np.cov(cnt)
    else:
        S_T2=(np.dot(cnt[:,:-tau[1]],cnt[:,tau[1]:].T)+np.dot(cnt[:,tau[1]:],cnt[:,:-tau[1]].T))/(cnt.shape[1]-1-tau[1])/2    
