...
from bbcpy.models.deep4 import Deep4Net
...
def run(baseline_mode, raw_args=None):
    ...
    model = Deep4Net(in_chans=INPUT_SHAPE[0],
                     n_classes=NUM_CLASSES,
                     input_window_samples=INPUT_SHAPE[1],
                     final_conv_length="auto",
                     n_filters_time=args.network.n_filters_time,
                     n_filters_spat=args.network.n_filters_spat,
                     filter_time_length=args.network.filter_time_length,
                     pool_time_length=args.network.pool_time_length,
                     pool_time_stride=args.network.pool_time_stride,
                     n_filters_2=args.network.n_filters_2,
                     filter_length_2=args.network.filter_length_2,
                     n_filters_3=args.network.n_filters_3,
                     filter_length_3=args.network.filter_length_3,
                     n_filters_4=args.network.n_filters_4,
                     filter_length_4=args.network.filter_length_4,
                     drop_prob=args.network.drop_prob,
                     double_time_convs=args.network.double_time_convs,
                     split_first_layer=args.network.split_first_layer,
                     batch_norm=args.network.batch_norm,
                     batch_norm_alpha=args.network.batch_norm_alpha,
                     stride_before_pool=args.network.stride_before_pool, )
    model.to(device)
    ...
    if args.pipeline_mode == "TESTING":
        ...
        #checkpoint = model.load_state_dict(torch.load(args.network.model_path))
        ...
        # bbcpy_trainer.eval_engine.add_event_handler(Events.EPOCH_COMPLETED, log_validation_results)
        ...
        plt = plot_cm(engine=bbcpy_trainer.eval_engine, class_type=args.data.task_type)
        plt.show()
