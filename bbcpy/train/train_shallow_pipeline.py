...
from bbcpy.models.shallow_fbcsp import ShallowFBCSPNet
...
def run(baseline_mode, raw_args=None):
    ...
    baseline_yaml_path = str(
        Path(root_dir) / "bbcpy" / "params" / "{:}.yaml".format("baselines_shallow")) + "::" + baseline_mode
    ...
    model = ShallowFBCSPNet(in_chans=INPUT_SHAPE[0],
                            ...
                            batch_norm=args.network.split_first_layer,
    ...
    if args.pipeline_mode == "TESTING":
        ...
        def confusion_matrix_fig():
            fig = plot_cm(engine=bbcpy_trainer.eval_engine, class_type=args.data.task_type)
