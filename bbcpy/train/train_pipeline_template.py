...
from bbcpy.models.nn_models_test import CNNModel_Test
from bbcpy.train.bbcpy_logger import log_training, save_model_summary, save_args_to_yaml, log_pBar
...
from bbcpy.utils.argparser import args_to_yaml, yaml_argparse, args_to_dict
...
def run(baseline_mode, raw_args=None):
    ...
    baseline_yaml_path = str(
        Path(root_dir) / "bbcpy" / "params" / "{:}.yaml".format("baselines_template")) + "::" + baseline_mode
    ...
    if args.pipeline_mode == "TRAINING":
        ...
        if args.tunning.enable_optuna_tuner:
            ...
            def objective(trial):
                ...
                model = CNNModel_Test(trial=trial)  # example
